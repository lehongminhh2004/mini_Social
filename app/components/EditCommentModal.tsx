...
export function EditCommentModal({ isOpen, onClose, comment, postId }: EditCommentModalProps) {
  ...
  // REF CHO KÉO THẢ ẢNH BÊN TRONG MODAL SỬA
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setExistingUrls(comment.mediaUrls || []);
      setNewFiles([]);
      setPreviewNewUrls([]);
    }
  }, [isOpen, comment]);
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (totalCount > 4) { alert("Chỉ được tải lên tối đa 4 ảnh!"); return; }
  ...
  const onMouseLeaveOrUp = () => { isDragging.current = false; };
          ...
          <motion.div initial={{ opacity: 0, y: '100%' }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} 
            ...
            <div className="p-4 flex gap-3 overflow-y-auto custom-scrollbar">
              ...
              <div className="flex-1 min-w-0">
                ...
                <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Reply..." className="w-full bg-transparent text-foreground placeholder-muted resize-none focus:outline-none min-h-[40px] mt-1 text-[15px]" rows={totalImages > 0 ? 2 : 4} autoFocus />

                {totalImages > 0 && (
                  <div ref={sliderRef} onMouseDown={onMouseDown} onMouseLeave={onMouseLeaveOrUp} onMouseUp={onMouseLeaveOrUp} onMouseMove={onMouseMove} className="mt-2 flex gap-3 overflow-x-auto pb-2 snap-x hide-scrollbar cursor-grab active:cursor-grabbing" style={{ WebkitOverflowScrolling: 'touch' }}>
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} disabled={totalImages >= 4} className="text-muted hover:text-foreground transition-colors p-2 -ml-2 rounded-full hover:bg-secondary disabled:opacity-50"><ImageIcon size={20} /></button>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 flex items-center justify-between border-t border-border shrink-0">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={handleSubmit} disabled={(!content.trim() && totalImages === 0) || content.length > maxLength || isSubmitting} className="bg-primary text-background px-5 py-1.5 rounded-full font-semibold flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
